...
//        zbTRoomPlan.setPlanId(zbTRoom.getRoomId()); 主键自增
        ...
        zbTRoomPlan.setZbStatus(1);
        //(3)创建房间配置信息，先获取配置信息，一并存入
        if (ZbConstant.Room.Type.ticket.name().equals(roomType)) {
            List<Object[]> list1=roomTypePriceDao.listByRoomPriceList(ZbConstant.Room.Type.ticket.name());
            ZbTQvodZbTRoomTypePrice roomTypePrice=roomTypePriceDao.findByTypeAndAndTime(ZbConstant.Room.Type.ticket.name(),mgrStartPlayRequestVo.getActivityTime());
            ...
            if (!ticketPR.contains(mgrStartPlayRequestVo.getPrice())) {
