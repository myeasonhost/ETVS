...
//            responseVo.setUserLevelRankList(userLevelRankList);
            ...
            if (zbTRoomConf == null) {
                throw new ServiceException("缓存无法获取当前房间的IM地址");
            }
    ...
    public IsChargedResponseVo isCharged(Integer userId, @PathVariable(value = "roomId") Integer roomId) throws ServiceException {
        try {
            ...
            if (!new Integer(1).equals(zbTRoomPlan.getZbStatus())){
                throw new ServiceException("主播未上线，表演还未开始，请稍后~");
            }
            ...
            responseVo.setZbId(zbTRoomPlan.getZbId());
            //当用户被禁用拉黑之后，不能进入房间测试
            HashMap<String,Object> hashMap=roomManager.bannedUser(userId,roomId);
            if (hashMap!=null){
                responseVo.setBannedInfo(hashMap);
                return responseVo;
            }
