define({ "api": [
  ...
  },
  {
    ...
    "type": "POST",
    "url": "/zhubo/changeRoom",
    "title": "更改房间类型信息",
    "name": "changeRoom",
    "description": "<blockquote> <p>主播API - 更改房间类型信息</br> 更改房间类型信息流程</br> （1）验证参数：是否合法</br> （2）验证房间状态：</br> A.未开播（=2）直接返回</br> B.直播中（=1）直接进入直播间，更新DB+缓存直播标题 等数据</br> （3）维护表：zb_t_room与zb_t_room_plan</br> A.查询当前房间场次，确保缓存中没场次（用户没有直播），直接返回</br> B.如果有场次，更新存储到缓存 房间信息，主播信息（主播昵称、等级、主播头像等），房间属性配置信息，场次信息</br> （4）组件返回值：</br> A.planId,status,result</br></p> </blockquote>",
    "parameter": {
      "fields": {
        "ticket": [
          {
          ...
          },
          {
            ...
            "optional": false,
            "field": "price",
            "description": "<p>门票单价=10,20,30,50|50,60,70,80|90,100,110,120  其中选择值</p>"
          }
        ],
        "time": [
          {
          ...
          },
          {
            ...
            "optional": false,
            "field": "price",
            "description": "<p>每分钟单价=10,20,30,50|50,60,70,80|90,100,110,120  其中选择值</p>"
          }
        ],
        "personal": [
          {
    ...
    "sampleRequest": [
      {
        "url": "http://192.168.0.138:8769/api/zhubo/changeRoom"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "type": "GET",
    "url": "/zhubo/getConfigInfo",
    "title": "获取房间配置信息",
    "name": "getConfigInfo",
    "description": "<blockquote> <p>主播API - 获取房间配置信息</br> 获取房间配置信息业务流程</br> （1）验证参数：是否合法</br> （2）获取主播权限：</br> A.判断主播是否被禁播；UI弹出提示框</br> B.判断是否拥有 时常房间、门票房间、私密房间、游戏房间的开播权限</br> （3）获取房间属性：</br> A.查库—动态配置开播时间、持续时间、门票价格数据等</br> B.配置UI—根据允许的类型从后台拉对应的房间配置数据，动态配置开播时间、持续时间、门票价格UI显示</br> （4）组建返回值：</br> A.如果有房间，返回上一次的房间配置信息（roomId,roomTitle,status,roomBackgroundImg），返回开房权限属性配置组装UI（Map）</br> B.如果无房间，返回当前的房间配置信息（roomId,status,roomTitle=null,roomBackgroundImg=null），返回开房权限属性配置组装UI（Map）</br> 实例： &quot;timeConf&quot;: { timeInterval&quot;: &quot;1|2|3&quot;, startTime&quot;: 1527143992429, activityTimeList&quot;: &quot;120|180|200&quot;, priceList&quot;: &quot;10,20,30,50|50,60,70,80|90,100,110,120&quot; } 使用间隔符&quot;|&quot;区分： 当下拉框选择：120的时候，对应的单价为：10,20,30,50，对应的时间间隔为：1 当下拉框选择：180的时候，对应的单价为：50,60,70,80，对应的时间间隔为：2 当下拉框选择：200的时候，对应的单价为：90,100,110,120，对应的时间间隔为：3</p> </blockquote>",
    "success": {
      "fields": {
        "Success 200": [
          {
        ...
        "ticket Success 200": [
          ...
          },
          {
            ...
            "description": "<p>继续时间=180|120|200</p>"
          },
          {
            "group": "ticket Success 200",
            "type": "String",
            "optional": false,
            "field": "priceList",
            "description": "<p>门票单价列表=10,20,30,50|50,60,70,80|90,100,110,120</p>"
          },
          {
            "group": "ticket Success 200",
            "type": "Integer",
        ...
        "time Success 200": [
          ...
          },
          {
            ...
            "optional": false,
            "field": "priceList",
            "description": "<p>每分钟单价列表=10,20,30,50|50,60,70,80|90,100,110,120</p>"
          },
          {
            "group": "time Success 200",
            "type": "Integer",
    ...
    "sampleRequest": [
      {
        "url": "http://192.168.0.138:8769/api/zhubo/getConfigInfo"
      }
    ]
  },
  {
