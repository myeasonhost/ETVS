...
import com.eason.live.zb.config.DnionLiveConfig;
...
@RestController
@RequestMapping("/dnion")
public class DnionUrlServiceImpl implements ILiveUrlService {
    @Autowired
    private RoomLiveDao roomLiveDao;
    @Autowired
    private DnionLiveConfig liveConfig;
    ...
    public String getPushUrl(String roomId) throws ServiceException {
        ...
        pushUrl=String.format(pushUrl,zbCode);
        ...
        zbRoomLive.setPushUrl(pushUrl);

        String playUrl=liveConfig.getPlayUrl().get(liveConfig.getLine());
        playUrl=String.format(playUrl,zbCode);
